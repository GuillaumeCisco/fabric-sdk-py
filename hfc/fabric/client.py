...
from hfc.fabric.block_decoder import BlockDecoder, \
    ...
    decode_fabric_endpoints, decode_proposal_response_payload, decode_signature_policy_envelope
...
class Client(object):
        ...
        if result:
            for k in key_path:
                try:
                    ...
                    _logger.warning(f'No key path {key_path} exists in net info')
                    return None
    ...
    async def channel_create(self, orderer, channel_name, requestor,
        ...
        if self.get_channel(channel_name):
            _logger.warning(f"channel {channel_name} already existed when creating")
            return False
    ...
    async def channel_join(self, requestor, channel_name, peers, orderer, orderer_admin):
        """
        Join a channel.
    ...
    async def get_channel_config(self, requestor, channel_name,
        ...
        for peer in peers:
            if isinstance(peer, Peer):
                ...
                _logger.error(f'{peer} should be a peer name or a Peer instance')
        ...
        for pplResponse in responses:
            try:
                if pplResponse.response and decode:
                    _logger.debug(
                        'response status {}'.format(pplResponse.response.status))
    ...
    async def chaincode_install(self, requestor, peers, cc_path, cc_name,
        ...
        if not target_peers:
            err_msg = "Failed to install chaincode: no functional peer provided"
            _logger.error(err_msg)
            raise Exception(err_msg)

        tran_prop_req = create_tx_prop_req(CC_INSTALL, cc_path, CC_TYPE_GOLANG,
                                           cc_name, cc_version, packaged_cc=packaged_cc)
        tx_context = create_tx_context(requestor, requestor.cryptoSuite,
                                       tran_prop_req)
    ...
    async def chaincode_instantiate(self, requestor, channel_name, peers,
        ...
        chaincode = {
            ...
            'escc': ccd.escc,
            'vscc': ccd.vscc,
            'policy': decode_signature_policy_envelope(ccd.policy.SerializeToString()),
            'data': {
                'hash': ccd.data.hash,
                'metadatahash': ccd.data.metadatahash,
            },
            'id': ccd.id,
            'instantiation_policy': decode_signature_policy_envelope(ccd.instantiation_policy.SerializeToString()),
        }
        return chaincode
