...
class Channel(object):
    ...
    def _build_principal(self, identity):
        ...
        newPrincipal.principal_classification = msp_principal_pb2.MSPPrincipal.ROLE
        ...
        if roleName == 'peer':
            ...
            raise Exception(f'Invalid role name found: must be one of "peer", "member" or "admin", but found "{roleName}"')
    ...
    def _check_policy(self, policy):
        ...
        if not 'identities' in policy or policy['identities'] == '' or not len(policy['identities']):
            raise Exception('Invalid policy, missing the "identities" property')
        elif not isinstance(policy['identities'], list):
            raise Exception('Invalid policy, the "identities" property must be an array')

        if not 'policy' in policy or policy['policy'] == '' or not len(policy['policy']):
            raise Exception('Invalid policy, missing the "policy" property')
    ...
    def _build_policy(self, policy, msps=None, returnProto=False):
        ...
        if policy:
            ...
            proto_signature_policy_envelope.rule.CopyFrom(self._get_policy(policy['policy']))
            proto_signature_policy_envelope.identities.extend([self._build_principal(x) for x in policy['identities']])
        else:
            ...
            for msp in msps:
                ...
                onePrn.principal_classification = msp_principal_pb2.MSPPrincipal.ROLE
            ...
            if len(principals) == 0:
                raise Exception('Verifying MSPs not found in the channel object, make sure "initialize()" is called first.')
    ...
    async def get_channel_config_with_orderer(self, tx_context, orderer):
        ...
        if 'index' in last_config['value']:
            seek_info = create_seek_info(last_config['value']['index'], last_config['value']['index'])
            seek_payload_bytes = create_seek_payload(seek_header, seek_info)
            sig = tx_context.sign(seek_payload_bytes)
            envelope = create_envelope(sig, seek_payload_bytes)
        ...
        if channel_header['type'] != common_pb2.CONFIG:
            raise Exception(f'Block must be of type "CONFIG" ({common_pb2.CONFIG}), but got "{channel_header["type"]}" instead')
